...
- Target Joomla 4.0+ and the namespace-based plugin architecture (this matches the v2.0.0 migration in git history). Do not reintroduce legacy non-namespaced patterns.
- Every class/method carries a docblock with `@since` tags and the GPL file header. Match the existing Joomla coding-standard formatting (tabs in `services/provider.php`, 4-space elsewhere as present).
